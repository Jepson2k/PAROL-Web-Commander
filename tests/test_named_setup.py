...
async def test_teach_saved_fixture_preview_and_execute_same_named_pose(
    ...
    await message("Captured the current TCP; use Set and Save to keep it")
    user.find(marker="setup-set-frame").click()
    await message("Set frame fixture; save to persist")
    ...
    await message("Set pose pick; save to persist")
    user.find(kind=ui.tab, content="Parameters").click()
    user.find(marker="setup-set-parameter").click()
    await message("Set parameter clearance; save to persist")
