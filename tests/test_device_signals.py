...
async def test_saved_named_output_readback_wait_cancellation_and_disconnection(
    ...
    try:
        ...
        result = await wait_signal.async_call(
            client, signal, False, timeout=0.2, poll_interval=0.03
        )
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as silent:
            ...
            absent = AsyncRobotClient(
                port=silent.getsockname()[1], timeout=0.02, retries=0
            )
            try:
                with pytest.raises(ConnectionError):
                    await wait_signal.async_call(
                        absent, signal, True, timeout=0.2, observation_timeout=0.05
                    )
            finally:
                await absent.close()
    finally:
